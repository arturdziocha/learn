...
public enum ReadabilityScores {
    ARI("Automated Readability Index",
            text -> 4.71 * text.getCharacters() / text.getWords() + 0.5 * text.getWords() / text.getSentences()
                    - 21.43),

    FK("Flesch–Kincaid readability tests",
            text -> 0.39 * text.getWords() / text.getSentences() + 11.8 * text.getSyllables() / text.getWords()
                    - 15.59),

    SMOG("Simple Measure of Gobbledygook",
            text -> 1.043 * sqrt(text.getPolysyllables() * 30. / text.getSentences()) + 3.1291),

    CL("Coleman–Liau index", text -> {
